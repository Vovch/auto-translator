...
Выполняйте команды из корня проекта, чтобы каталоги `./input` и `./output` стали доступны в контейнере как `/workspace/input` и `/workspace/output`. **Обратите особое внимание на имена файлов, которые вы указали в качестве параметров `--input` и `--output`!**.
